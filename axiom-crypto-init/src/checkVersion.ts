...
export const checkVersion = async (): Promise<void> => {
  ...
  try {
    ...
    const { stdout } = await exec(`npx create-axiom-client@latest --version`, { timeout: 200 });
    version = stdout;
  }
  catch (_e) {
  ...
  const isCurrentVersion = semver.gte(CURRENT_VERSION, remoteVersion);
  if (!isCurrentVersion) {
    ...
    console.log(`Removed old installation. Please try running ${chalk.bold("npx create-axiom-client")} again.`);
    process.exit();
  }
}